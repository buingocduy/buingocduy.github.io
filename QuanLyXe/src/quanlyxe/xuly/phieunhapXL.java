...
public class phieunhapXL {
    public ArrayList<phieunhapTT> getAllRecords(){
        ...
        while(rs.next()){
            phieunhapTT phieunhap = new phieunhapTT(rs.getString("MaPN"),rs.getString("Username"),rs.getString("MaNCC"),rs.getDate("NgayNhap"),rs.getInt("MaKho"));
            list.add(phieunhap);
        }
        } catch (Exception e) {
    ...
    //Them
    public int AddNewRecord(String Username, String MaNCC, Date ngaynhap, int maKho){
        int rowCount = 0;
        try {
    ...
    //Sua
    public int UpdateRecord(String MaPN,String Username, String MaNCC, Date ngaynhap, int maKho){
        int rowCount = 0;
        try {
            SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");  
            String strNgayNhap = formatter.format(ngaynhap); 
