 ...
 * @author BND6699
 */
public class UpdateHangIO {
    Connection conn = null; // kết nối với sql
    PreparedStatement ps = null; // lấy lệnh từ netbeans qua sql
    ResultSet rs = null; //nhận kết quả trả về
    
    public Hang checkUpdate(String tenhang, String logo){     
    try {
            String query = "update hang set logo = ? where tenhang = ?";
            conn = new DBContext().getConnection();//mở kết nối sql
            ps = conn.prepareStatement(query);//truyền lệnh cho sql
            ps.setString(2, tenhang); //truyền vào dấu ? taikhoan
            ps.setString(1, logo); //truyền vào dấu ? taikhoan
